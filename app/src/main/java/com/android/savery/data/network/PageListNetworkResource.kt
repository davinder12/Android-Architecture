...
import ywaste.com.ywaste.ui.util.IListResource
...
abstract class PagedListNetworkResource<LocalType, Key>(
 ...
 abstract class NetworkListResourceBoundaryCallback<LocalType, RemoteType>(private val appExecutors: AppExecutors) : PageListNetworkResource<LocalType>(){
     ...
     protected  abstract fun mapToLocal(items: RemoteType): List<LocalType>
      ...
      override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, LocalType>) {
          ...
          try {
              ...
              retry = { loadInitial(params, callback) }
              networkState.postValue(NetworkState.error(ioException.message, R.string.network_error_unknown))
         ...
         networkStateAfter.postValue(NetworkState.loading)
          loadAfter(params.key).enqueue(object : retrofit2.Callback<RemoteType> {
             override fun onFailure(call: Call<RemoteType>, t: Throwable) {
                 ...
                 networkState.postValue(NetworkState.error(t.message, R.string.network_error_unknown))
